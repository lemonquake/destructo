...
export class ParticleSystem {
  ...
  muzzleFlash(position,direction,weapon){
    if(!position)return;
    const isTankShell = (weapon.projectileStyle || '').startsWith('tank_shell_');
    const energy=['plasma','arc','tank_shell_yellow','tank_shell_blue'].includes(weapon.projectileStyle),rocket=['rocket','missile','tank_shell_brown','tank_shell_red'].includes(weapon.projectileStyle),flame=weapon.projectileStyle==='plasma'&&weapon.shotPower<30,launch=Boolean(weapon.mine);
    const dir=(direction?.lengthSq?.()>0?direction:new THREE.Vector3(0,0,1)).clone().normalize();
    const flash=this.muzzleFlashes.find(item=>item.life<=0);if(flash){flash.life=flash.maxLife=flame ? .11 : isTankShell ? .12 : rocket ? .09 : energy ? .08 : .07;flash.mesh.visible=true;flash.mesh.position.copy(position);flash.mesh.material.color.setHex(energy?weapon.color:flame?0xff6428:launch?0x9eff8a:0xffdc62);flash.mesh.material.opacity=1;flash.mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir);const scale=(launch?.65:flame?1.75:isTankShell?1.85:rocket?1.35:energy?1.15:weapon.pellets?1.25:1)*1.3;flash.mesh.scale.set(scale,scale*(flame?1.8:1),scale);flash.mesh.rotateY(Math.random()*Math.PI*2)}
    const core=this.flashCores.find(item=>item.life<=0);
    if(core){core.life=core.maxLife=flame?.09:isTankShell?.08:.06;core.mesh.visible=true;core.mesh.position.copy(position).addScaledVector(dir,.12);core.mesh.material.color.setHex(energy?weapon.color:flame?0xff8a3c:launch?0x9eff8a:0xffe9a8);core.mesh.material.opacity=1;core.startScale=(launch?.5:isTankShell?1.7:rocket?1.3:flame?1.2:weapon.pellets?1.15:.85)*(.85+Math.random()*.35);core.mesh.scale.setScalar(core.startScale);core.mesh.quaternion.copy(this.cameraQuaternion)}
    // a few hot sparks kicked forward out of the barrel
    if(this.quality>=.7&&!launch){const sparkCount=2+(isTankShell?4:weapon.pellets?2:0);for(let n=0;n<sparkCount;n++){const s=this.fragments.find(x=>x.life<=0);if(!s)break;s.isWater=false;s.life=s.maxLife=.12+Math.random()*.18;s.mesh.visible=true;s.mesh.material.color.setHex(n%2?0xffe66c:weapon.color||0xffab3f);s.mesh.material.opacity=1;s.mesh.position.copy(position);s.mesh.scale.setScalar(.14+Math.random()*.25);s.velocity.copy(dir).multiplyScalar(8+Math.random()*8);s.velocity.x+=(Math.random()-.5)*4;s.velocity.y+=Math.random()*2.2;s.velocity.z+=(Math.random()-.5)*4}}
