...
export class Game{
  ...
  bindUI(){this.screen.addEventListener('click',e=>{const action=e.target.closest('[data-action]')?.dataset.action;if(!action)return;
    ...
    if(action==='custom-match')this.showGameSetup();else if(action==='deploy'){const limit=ALL_MAPS[this.selectedMap]?.maxTeams||MAX_PLAYERS;if(this.setup.length<=limit)this.startMission('skirmish')}else if(action==='missions')this.showMissions();else if(action==='dimension:prev')this.cycleDimension(-1);else if(action==='dimension:next')this.cycleDimension(1);else if(action.startsWith('mission:')){const id=action.slice(8),mission=CAMPAIGN_MISSIONS[id];if(mission&&(!mission.requires||this.save.campaignCompleted(mission.requires)))this.startMission(id)}else if(action==='hub')this.showHub();else if(action==='menu')this.showMenu();else if(action==='settings')this.showSettings();else if(action==='shop')this.showShop();else if(action==='dbuild')this.showDBuild();else if(action==='casino')this.playCasino();else if(action==='reset'){this.save.reset();this.league=new LeagueSystem([]);this.showMenu()}
    ...
    else if(action==='rankings'){this.rankingsReturnState=this.state==='hub'?'hub':'menu';this.showRankings()}else if(action==='rankings:back'){this.rankingsReturnState==='hub'?this.showHub():this.showMenu()}else if(action.startsWith('rankings:')){this.rankingsMode=action.slice(9);this.showRankings()}else if(action.startsWith('gear:'))this.buyGear(action.slice(5));else if(action.startsWith('cos:'))this.handleCosmetic(action.slice(4))});
  ...
  setupCampaignMission(){
    ...
    this.goldenDelivered=false;this.goldenCrate=null;this.extractionPoint=null;this.extractionCircle=null;this.jetDropPoint=null;this.campaignTargets=[];this.campaignCountedDeaths=new Set();document.body.classList.add('campaign-mode');document.getElementById('quest-hud')?.classList.remove('hidden');document.getElementById('sudden-death-timer')?.classList.toggle('hidden',!this.campaignTimer);document.getElementById('crate-forecast')?.classList.toggle('hidden',this.mission.id==='four-of-a-kind');
  ...
  updateCampaignRespawn(dt){const team=this.teamMap[this.playerTeam];if(this.livingUnits(this.playerTeam).length)return this.campaignRespawnTimer=this.mission.rules?.reinforcementSeconds||5;this.campaignRespawnTimer-=dt;const overlay=document.getElementById('respawn-overlay'),counter=document.getElementById('respawn-countdown');overlay?.classList.remove('hidden');if(counter)counter.textContent=Math.max(0,Math.ceil(this.campaignRespawnTimer));if(this.campaignRespawnTimer>0)return;this.campaignRespawnTimer=this.mission.rules?.reinforcementSeconds||5;const unit=this.addUnit(this.mission.id==='golden-shield'?'heavy':'commando',team.id,this.respawnPos(team.id,0));if(this.mission.startingWeapon)this.equipPrimaryWeapon(unit,this.mission.startingWeapon,WEAPONS[this.mission.startingWeapon],this.mission.startingAmmo||300,0);this.possess(unit);overlay?.classList.add('hidden');this.particles.burst(unit.group.position.clone().add(new THREE.Vector3(0,1,0)),team.color,32,8);this.hud.toast('5-SECOND REINFORCEMENT DEPLOYED')}
  updateGoldenShield(dt){
    this.campaignTimer=Math.max(0,this.campaignTimer-dt);const timer=document.getElementById('sudden-death-timer'),value=document.getElementById('timer-val');timer?.classList.remove('hidden');if(value){value.textContent=`${String(Math.floor(this.campaignTimer/60)).padStart(2,'0')}:${String(Math.ceil(this.campaignTimer%60)).padStart(2,'0')}`;value.style.color=this.campaignTimer<60?'#ff5266':'#ffd23f'}this.quest.progress(600-this.campaignTimer,600);this.updateCampaignRespawn(dt);
  ...
  updateCampaignMission(dt,time){
    ...
    else if(this.mission.id==='gold-rush')this.updateGoldRushMission(dt,step);else if(this.mission.id==='golden-shield')this.updateGoldenShield(dt);else this.updateCampaignAssault(dt,step)
  }
  ...
  updatePlayer(dt){if(this.input.consume('Tab')){if(this.player?.mountedTurret||this.player?.mountedBunker||this.player?.mountedMotorcycle)this.exitInteractive(this.player);else this.switchPlayer()}const p=this.player;if(p.dead)return;if(!this.input.mouse.down)this._mouseCaptureClick=false;this.updateViewModeInput(p);p.fireCooldown=Math.max(0,p.fireCooldown-dt);if(p.stun>0){p.stun-=dt;p.group.position.addScaledVector(p.velocity,dt);p.velocity.multiplyScalar(Math.pow(.15,dt));return}
    ...
    const jump=stepJumpState({motion:p.state||'grounded',verticalVelocity:p.verticalVelocity||0,jetpackBurn:p.jetpackBurn||0,mp:p.mp},{grounded,jumpPressed:this.input.consume('Space'),jumpHeld:this.input.keys.has('Space'),hasJetpack:Boolean(p.jetpack),blocked:Boolean(p.carriedCrate?.questHeavy),jumpScale:p.passive?.id==='highjumper'?1.4:1},dt);
    p.state=jump.motion;p.verticalVelocity=jump.verticalVelocity;p.jetpackBurn=jump.jetpackBurn;p.jetpackActive=jump.jetpackActive;p.mp=jump.mp;
    if(jump.jumped){this.audio.play('pickup',1.6);if(this.world.isWater(p.group.position))this.particles.waterSplash(p.group.position,5,12,1.1)}
